...
source "../../../config/params_mengxiao.sh"
...
do 
    ...
    qsub -N "${name}_${s}" \
        ...
        -pe smp $n_cores \
        -l mem_requested=8G \
        $run_script_file $function_script_file $project_dir $h5ad_dir $s $min_count $min_gene $min_spots $environment

done


###        $run_script_file $function_script_file $raw_data_dir $h5ad_dir $project_dir $s $environment