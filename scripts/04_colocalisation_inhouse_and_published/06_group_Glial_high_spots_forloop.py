...
for section_name in filtered_section_names:
    ...
    qc_dir = os.path.join(res_dir, '05_zoom_in_HandE', 'figures', 'PNS_Glial_score_per_sample', today)
    os.makedirs(qc_dir, exist_ok=True)
    # Define PDF file path
    pdf_path = os.path.join(qc_dir, f'{section_name}_combined_plots.pdf')
    with PdfPages(pdf_path) as pdf:
        # Plot 1: Plot the PNS_glial model score + spots above a certain threshold
        cmap = "YlOrRd"
        ...
        # TLS model results
        sc.pl.spatial(adata, color='PNS_glial_score', size=spot_size, cmap=cmap_adj, bw=False, alpha_img=1, crop_coord=coords, title=f'PNS_glial score', vmin=0, ax=axs[0])
        ...
        plt.suptitle(f'{section_name} - PNS_glial score + PNS_glial-high spots', y=1.05, fontweight='bold', x=0.5)
        ...
        if len(adata_sub) == 0 or len(adata_sub) == 1:
            ...
            out_dir_csv = os.path.join(res_dir, '05_zoom_in_HandE', 'csv_files', 'PNS_Glial_score_per_sample', today)
            os.makedirs(out_dir_csv, exist_ok=True)
            to_save_df = adata_sub.obs[['PNS_glial_score']]
            to_save_df['group'] = ''
            to_save_df['group_singleton'] = ''
            filename = f'{section_name}_PNS_glial_groups.csv'
        ...
        plt.suptitle(f'{section_name} - neighborhood graph of spots with PNS_glial score >{threshold})', y=1.1, fontweight='bold', x=0.5)
        ...
        plt.suptitle(f'{section_name} - grouping of spots with PNS_glial score >{threshold})', y=1.05, fontweight='bold', x=0.5)
        ...
        adata_sub.obs['PNS_glial_group'] = temp_df['group']
    ...
    out_dir_csv = os.path.join(res_dir, '05_zoom_in_HandE', 'csv_files', 'PNS_Glial_score_per_sample', today)
    os.makedirs(out_dir_csv, exist_ok=True)
    to_save_df = adata_sub.obs[['PNS_glial_score', 'group', 'group_singleton', 'PNS_glial_group']]
    to_save_df.columns = ['PNS_glial_score', 'all_groups', 'group_singleton', 'group_singleton_removed']
    filename = f'{section_name}_PNS_glial_groups.csv'
    to_save_df.to_csv(os.path.join(out_dir_csv, filename))
