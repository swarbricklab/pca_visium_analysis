...
TLS_group_dir = os.path.join(TLS_res_dir, 'TLS_plots_per_sample', f'threshold_{threshold_adj}', 'csv_files')
...
# Read in the results & add to anndata
TLS_file_name = f'{sample_id}_TLS_groups.csv'
...
image_dir = os.path.join(project_dir, 'data', 'images', joined_id)
image_file_path = glob.glob(os.path.join(image_dir, '*.tif'))[0]
...
size_fact = img.shape[0]/adata_sub.uns['spatial'][sample_id]['images']['hires'].shape[0]

# Extract the downsampled hires scale factor
hires_fact = adata_sub.uns['spatial'][sample_id]['scalefactors']['tissue_hires_scalef']

# Calculate the scale factor for the original hires image and add to the anndata object
adata_sub.uns['spatial'][sample_id]['scalefactors'][f'tissue_{original_hires_key}_scalef'] = size_fact * hires_fact

# -- Add scale factor to original anndata as well
adata.uns['spatial'][sample_id]['scalefactors'][f'tissue_{original_hires_key}_scalef'] = size_fact * hires_fact
...
# Save plots
out_dir_plots = os.path.join(out_dir, 'plots', 'zoom', today)
os.makedirs(out_dir_plots, exist_ok=True)

# Specify a filename for saving the plot
filename = f'{sample_id}_TLS_groups_zoom.pdf'
...
spot_diameter_fulres = adata.uns['spatial'][sample_id]['scalefactors']['spot_diameter_fullres']
...
with PdfPages(os.path.join(out_dir_plots, filename)) as pdf:
    for i, group in enumerate(all_groups):
        ...
        sc.pl.spatial(adata, color='TLS_model', groups=[group], bw=False, alpha_img=1, crop_coord=coords, title=f'{sample_id} - {group}',
        ...
        ax=axs[n]
        # Plot the group on H&E hires image
        sc.pl.spatial(adata_group_sub, color='group_singleton_removed', groups=[group], bw=False, alpha_img=1, crop_coord=coords, title=f'{sample_id} - {group}',
        ...
        adata_crop.uns['spatial'][sample_id]['images'][original_hires_key] = crop_img
        ...
        n+=1
        ax=axs[n]
        sc.pl.spatial(adata_crop, color='group_singleton_removed', groups=[group], bw=False, alpha_img=1, crop_coord=(xmin, width_crop, ymin, height_crop), title=f'{sample_id} - {group}',
        ...
        # -- Plot the TLS score within the zoom area
        n+=1
        ax=axs[n]
        sc.pl.spatial(adata_crop, color='TLS_model', bw=False, alpha_img=1, crop_coord=(xmin, width_crop, ymin, height_crop), ax=ax, cmap=cmap_adj, vmin=0, vmax=vmax, title=f'{group} - TLS model',
        ...
        scalebar = ScaleBar(microns_per_pixel, units='µm', location='lower right', box_alpha=0.5)
        ax.add_artist(scalebar)
        plt.suptitle(f'{sample_id} ({subtype}) - grouping of spots with TLS score >{threshold}) - {group}', y=1.1, fontweight='bold', x=0.5)
