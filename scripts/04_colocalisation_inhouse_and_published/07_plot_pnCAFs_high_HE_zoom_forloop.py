...
score_dir = "/share/ScratchGeneral/evaapo/projects/PCa_Visium/pca_visium_analysis/results/04_colocalisation_inhouse_and_published/05_zoom_in_HandE/csv_files/pnCAFs_score_per_sample/20240730/"
...
TLS_file_name = f'{section_name}_pnCAFs_groups.csv'
...
# Subset anndata to pnCAFs score above threshold
...
qc_dir = os.path.join(res_dir, 'pnCAFs_score')
...
filename = f'{section_name}_pnCAFs_groups_zoom.pdf'
...
with PdfPages(os.path.join(out_dir_plots, filename)) as pdf:
    for i, group in enumerate(all_groups):
        ...
        sc.pl.spatial(adata, color='pnCAFs_score', groups=[group], bw=False, alpha_img=1, crop_coord=coords, title=f'{section_name} - {group}',
        ...
        # -- Plot the pnCAFs score within the zoom area
        n+=1
        ax=axs[n]
        sc.pl.spatial(adata_crop, color='pnCAFs_score', bw=False, alpha_img=1, crop_coord=(xmin, width_crop, ymin, height_crop), ax=ax, cmap=cmap_adj, vmin=0, vmax=vmax, title=f'{group} - pnCAFs score',
        ...
        scalebar = ScaleBar(microns_per_pixel, units='µm', location='lower right', box_alpha=0.5)
        ax.add_artist(scalebar)
        plt.suptitle(f'{section_name} - grouping of spots with pnCAFs score >{threshold}) - {group}', y=1.1, fontweight='bold', x=0.5)
