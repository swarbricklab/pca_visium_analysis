...
analysis = "19_colocalisation_major_temp"
...
# P15-2   P25 P2-2 P3-1 P4-1 P4-2 P10-1 P10-2 P11-1 P13-2
...
for (section_name in unique(samples_config_sub$section_name)){
  ...
  prop_df <- read.csv(paste0("/share/ScratchGeneral/evaapo/projects/PCa_Visium/pca_visium_analysis/results/02_cell2location/17a_cell_type_mapping_major_temp/", section_name, "/objects/q05_cell_abundance_w_sf.csv"))
...
df_tidy <- df %>% pivot_longer(celltypes, names_to = "celltype_major_temp", values_to = "c2l_value")
...
#  7 D1_P4_C2                          1477
#  8 D1_P10_C2                          1248
#  9 PCaP4_C1_P13_C2                 4753
# 10 PCaP10_C1_P3_C1                 4247
...
spread_df <- spread(merged_df, key = celltype_major_temp, value = c2l_value)
...
cell_types_of_interest <- c("uniCAFs", "NPF_like", "pnCAFs", "whCAFs", "PNS_glial", "Epithelial", "Endothelial", "SMCs")
...
pdf(paste0(figDir, "celltype_major_temp_cell2location.pdf"), width = 15, height=25)
print(p)
dev.off()
...
# Print the heatmap plot
pdf(paste0(figDir, "celltype_major_temp_cell2location_clustered.pdf"), width = 15, height=25)
print(p)
dev.off()
...
p <- pheatmap(
  ...
  border_color = NA,
  cellwidth = 11, cellheight = 14, 
  main = "Pearson correlation heatmap\nCell2Location - cell type major temp",
  ylab = "Cell type Pair",
  xlab = "Sample IDs",
  scale = "none"
)
...
# Print the heatmap plot
pdf(paste0(figDir, "celltype_major_temp_cell2location_histology.pdf"), width = 15, height=25)
print(p)
dev.off()
...
# Print the heatmap plot
pdf(paste0(figDir, "celltype_major_temp_cell2location_clustered_histology.pdf"), width = 15, height=25)
print(p)
dev.off()
